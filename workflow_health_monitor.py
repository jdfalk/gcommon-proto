...
class WorkflowHealthMonitor:
    ...
    def get_workflow_runs(self, limit: int = 50) -> List[Dict]:
        ...
        success, output = self._run_gh_command(
            [
                ...
                str(limit),
                "--json",
                "id,status,conclusion,workflowName,createdAt,headBranch",
            ]
        )
    ...
    def analyze_workflow_health(self, runs: List[Dict]) -> Dict[str, Dict]:
        ...
        for workflow_name, workflow_runs in workflows.items():
            ...
            failed_runs = sum(
                1 for run in workflow_runs if run.get("conclusion") == "failure"
            )
            ...
            for run in sorted(
                workflow_runs, key=lambda x: x.get("createdAt", ""), reverse=True
            ):
            ...
            health_metrics[workflow_name] = {
                ...
                "last_run_status": workflow_runs[0].get("conclusion")
                if workflow_runs
                else "unknown",
            }
        ...
        for workflow_name, metrics in health_metrics.items():
            if metrics["status"] == "critical":
                ...
                if workflow_name not in self.known_failures:
                    ...
                    self.failure_counts[workflow_name] = (
                        self.failure_counts.get(workflow_name, 0) + 1
                    )
    ...
    def generate_health_report(self, health_metrics: Dict[str, Dict]) -> str:
        ...
        report_lines.extend(["", "## Workflow Details", ""])
...
Time: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        ...
        logger.critical(
            f"ALERT: {workflow_name} - {metrics['consecutive_failures']} consecutive failures"
        )
    ...
    def run_health_check(self) -> Dict[str, Dict]:
        ...
        critical_count = sum(
            1 for m in health_metrics.values() if m["status"] == "critical"
        )
        warning_count = sum(
            1 for m in health_metrics.values() if m["status"] == "warning"
        )

        logger.info(
            f"Health check complete: {total_workflows} workflows, {critical_count} critical, {warning_count} warnings"
        )
...
The monitor will:
    ...
    # Create monitor
    monitor = WorkflowHealthMonitor(repo=args.repo, check_interval=args.interval)
