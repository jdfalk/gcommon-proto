...
class ProtoAnalyzer:
    def __init__(self, base_path: Path):
        ...
        self.type_definitions: Dict[str, List[Path]] = {}  # type_name -> [files that define it]
        self.file_imports: Dict[Path, Set[str]] = {}  # file -> set of imported files
        self.file_types: Dict[Path, Set[str]] = {}  # file -> set of types defined in file
        self.missing_imports: Dict[str, List[Tuple[Path, List[str]]]] = defaultdict(list)
    ...
    def build_type_index(self):
        ...
        print(f"   Found {len(self.type_definitions)} unique types across {len(self.proto_files)} files")
                        ...
                        for defining_file in defining_files:
                            ...
                            try:
                                ...
                                if existing_entry:
                                    ...
                                    self.missing_imports[import_path_str].append((file_path, [ref_type]))
                            except ValueError:
                                # File is outside base_path
                                continue
                    else:
                        # Type not found anywhere - might be an external type or error
                        print(f"⚠️  Type '{ref_type}' referenced in {file_path.relative_to(self.base_path)} but not found anywhere")
    ...
    def generate_report(self):
        ...
        sorted_imports = sorted(
            self.missing_imports.items(),
            key=lambda x: len(x[1]),
            reverse=True
        )
        
        print(f"Found {len(self.missing_imports)} proto files that need to be imported")
        print(f"Total files with missing imports: {sum(len(files) for _, files in self.missing_imports.items())}")
    ...
    def generate_fix_commands(self):
        ...
        script_lines = ["#!/bin/bash", "# Auto-generated script to fix missing proto imports", ""]
        
        for file_path, needed_imports in file_fixes.items():
            script_lines.append(f"# Fix imports for {file_path.relative_to(self.base_path)}")
            
            for import_path in needed_imports:
                # Add import after the last existing import or after package declaration
                sed_cmd = f'sed -i \'/^import.*proto";$/a import "{import_path}";' + f"' '{file_path}'"
