...
"""
Script to dismiss code scanning alerts for unused imports in the SDK folder.

This script uses the GitHub CLI to:
1. List all open code scanning alerts for unused imports in the sdks/ folder
...
Options:
    ...
    --reason        Dismiss reason (default: "used in library")
...
def get_unused_import_alerts() -> List[Dict[str, Any]]:
    """Get all open code scanning alerts for unused imports in sdks/ folder."""
    # Get all open alerts
    alerts = run_gh_command(
        [
            ...
            "--jq",
            '.[] | select(.state == "open" and (.most_recent_instance.location.path | startswith("sdks/")) and (.rule.id | contains("unused-import") or contains("unused-global-variable"))) | {number, state, rule: .rule.id, message: .message.text, path: .most_recent_instance.location.path, url}',
        ]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Dismiss code scanning alerts for unused imports in SDK folder"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--reason",
        default="used in library",
        choices=["false positive", "used in library", "wont fix"],
        help="Dismiss reason (default: used in library)",
    )

    args = parser.parse_args()

    print("🔍 Fetching code scanning alerts for unused imports in sdks/ folder...")
    alerts = get_unused_import_alerts()

    if not alerts:
        print("✅ No unused import alerts found in sdks/ folder")
        return

    print(f"📋 Found {len(alerts)} unused import alert(s) in sdks/ folder:")
    print()
    ...
    for alert in alerts:
        ...
        if args.dry_run:
            ...
            comment = "Auto-dismissed: SDK generated files often have unused imports that are part of the protobuf generation process"
