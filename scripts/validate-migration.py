...
class MigrationValidator:
        ...
        try:
            ...
            if not package_name.startswith("gcommon.v1."):
                self.warnings.append(
                    f"Package name doesn't follow convention: {package_name} in {proto_file}"
                )
            ...
            if not go_package.startswith("github.com/jdfalk/gcommon/pkg/"):
                self.warnings.append(
                    f"go_package doesn't follow convention: {go_package} in {proto_file}"
                )
            ...
            for import_path in imports:
                if import_path.startswith("pkg/"):
                    self.errors.append(
                        f"Old import path found: {import_path} in {proto_file}"
                    )
                    return False
    ...
    def validate_no_old_files(self) -> bool:
        ...
        if old_proto_files:
            if is_dry_run:
                print(
                    f"ℹ️  Found {len(old_proto_files)} old proto files (expected in dry-run mode)"
                )
                return True
            else:
                self.warnings.extend(
                    [f"Old proto file still exists: {f}" for f in old_proto_files]
                )
