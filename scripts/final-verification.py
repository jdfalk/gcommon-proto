...
class FinalVerifier:
    ...
    def verify_no_old_files(self) -> bool:
        ...
        if self.detect_dry_run_mode():
            ...
            if old_files:
                print(
                    f"ℹ️  Found {len(old_files)} old proto files (expected in dry-run mode)"
                )
            return True
        ...
        if old_files:
            self.warnings.extend(
                [
                    f"Old proto file still exists: {f.relative_to(self.root_dir)}"
    ...
    def verify_build_system(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["buf", "--version"], capture_output=True, text=True, timeout=10
            )
        ...
        try:
            result = subprocess.run(
                ["buf", "lint"], capture_output=True, text=True, timeout=30
            )
            if result.returncode != 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Final verification for proto migration"
    )
