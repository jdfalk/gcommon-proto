...
class ProtoMigrator:
    ...
    def discover_proto_files(self, domain_filter: Optional[str] = None) -> List[Path]:
        ...
        for proto_file in self.root_dir.rglob("*.proto"):
            if "pkg/" in str(proto_file) and "/proto/" in str(proto_file):
                ...
                pkg_index = next(
                    i for i, part in enumerate(path_parts) if part == "pkg"
                )
    ...
    def analyze_proto_file(self, file_path: Path) -> ProtoFile:
        ...
        try:
            ...
            go_package_match = re.search(
                r'option\s+go_package\s*=\s*"([^"]+)";', content
            )
            current_go_package = (
                go_package_match.group(1).strip() if go_package_match else ""
            )
            ...
            if domain in self.domain_mapping:
                ...
                new_package = self.generate_new_package_name(
                    target_path, mapping["package_prefix"]
                )
                new_go_package = self.generate_new_go_package(
                    target_path, mapping["go_package_prefix"]
                )
            else:
    ...
    def determine_target_path(
        self, current_path: str, domain: str, content: str
    ) -> str:
        """Determine the target path for a proto file."""
        filename = Path(current_path).name

        if domain not in self.domain_mapping:
            return f"gcommon/v1/common/types/{filename}"
    ...
    def update_proto_content(self, content: str, proto_file: ProtoFile) -> str:
        ...
        # Update file header
        content = re.sub(
            r"// file: [^\n]+", f"// file: {proto_file.target_path}", content
        )

        # Update package declaration
        content = re.sub(
            r"package\s+[^;]+;", f"package {proto_file.package_name};", content
        )
            ...
            if len(parts) >= 2:
                ...
                if domain in self.domain_mapping:
                    ...
                    if "service" in filename.lower():
                        return f"gcommon/v1/{domain}/services/{filename}"
                    elif any(
                        word in filename.lower() for word in ["enum", "status", "type"]
                    ):
                        return f"gcommon/v1/{domain}/types/{filename}"
                    elif any(
                        word in filename.lower() for word in ["config", "setting"]
                    ):
                        return f"gcommon/v1/{domain}/config/{filename}"
                    elif any(
                        word in filename.lower()
                        for word in ["api", "request", "response"]
                    ):
                        return f"gcommon/v1/{domain}/api/{filename}"
                    else:
                        return f"gcommon/v1/{domain}/messages/{filename}"
    ...
    def migrate_file(self, proto_file: ProtoFile) -> bool:
        ...
        try:
            ...
            if self.dry_run:
                print(
                    f"📝 Would migrate: {proto_file.current_path} -> {proto_file.target_path}"
                )
                return True
    ...
    def run_migration(self, domain_filter: Optional[str] = None) -> MigrationStats:
        ...
        for file_path in proto_files:
            ...
            if proto_file.issues:
                print(
                    f"⚠️  Issues in {proto_file.current_path}: {', '.join(proto_file.issues)}"
                )
