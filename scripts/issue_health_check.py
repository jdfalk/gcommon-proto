...
def check_issue_system_health(
    directory: str = ".github/issue-updates",
) -> Dict[str, any]:
                    ...
                    if guid:
                        if guid in seen_guids:
                            results["warnings"].append(
                                f"Duplicate GUID in processed: {guid}"
                            )
        ...
        try:
            with open(file_path, "r") as f:
                ...
                if guid:
                    if guid in seen_guids:
                        results["errors"].append(
                            f"Duplicate GUID found: {guid} in {file_path.name}"
                        )
                        results["stats"]["duplicate_guids"] += 1
                ...
                if action in ["comment", "update", "close"] and (
                    ...
                    results["warnings"].append(
                        f"Invalid issue number in {file_path.name}: {issue_number}"
                    )
    ...
    results["stats"]["total_files"] = (
        results["stats"]["processed_files"] + results["stats"]["pending_files"]
    )
...
def print_health_report(results: Dict[str, any]) -> None:
    ...
    print("\n🏥 Issue System Health Check")
    print(
        f"Status: {status_emoji.get(results['status'], '❓')} {results['status'].upper()}"
    )
    ...
    if args.auto_fix and results["status"] != "healthy":
        ...
        try:
            ...
            if repair_script.exists():
                subprocess.run(
                    [sys.executable, str(repair_script), "--auto-commit"], check=True
                )
