...
class ImportPathFixer:
    ...
    def scan_proto_files(self):
        ...
        if PKG_DIR.exists():
            for proto_file in PKG_DIR.rglob("*.proto"):
                self.proto_file_map[proto_file.name] = str(proto_file.relative_to(PROJECT_ROOT))
    ...
    def fix_go_package(self, content: str, proto_path: Path) -> str:
        ...
        try:
            ...
            if len(path_parts) >= 3 and path_parts[0] == 'pkg' and path_parts[2] == 'proto':
                domain = path_parts[1]
            ...
            elif len(path_parts) >= 5 and path_parts[0] == 'proto' and path_parts[1] == 'gcommon':
                domain = path_parts[3]
                category = path_parts[4] 
                new_go_package = f"github.com/jdfalk/gcommon/proto/gcommon/v1/{domain}/{category}"
