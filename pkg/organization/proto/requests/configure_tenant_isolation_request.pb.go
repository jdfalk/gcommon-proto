// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: pkg/organization/proto/requests/configure_tenant_isolation_request.proto

//go:build !protoopaque

package organizationpb

import (
	pb "../../../../pkg/common/pb"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ConfigureTenantIsolationRequest struct {
	state protoimpl.MessageState `protogen:"hybrid.v1"`
	// Request metadata for tracing and context
	Metadata *pb.RequestMetadata `protobuf:"bytes,1,opt,name=metadata" json:"metadata,omitempty"`
	// Tenant identifier
	TenantId *string `protobuf:"bytes,2,opt,name=tenant_id,json=tenantId" json:"tenant_id,omitempty"`
	// Isolation configuration to apply
	Isolation     *TenantIsolation `protobuf:"bytes,3,opt,name=isolation" json:"isolation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConfigureTenantIsolationRequest) Reset() {
	*x = ConfigureTenantIsolationRequest{}
	mi := &file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfigureTenantIsolationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfigureTenantIsolationRequest) ProtoMessage() {}

func (x *ConfigureTenantIsolationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *ConfigureTenantIsolationRequest) GetMetadata() *pb.RequestMetadata {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *ConfigureTenantIsolationRequest) GetTenantId() string {
	if x != nil && x.TenantId != nil {
		return *x.TenantId
	}
	return ""
}

func (x *ConfigureTenantIsolationRequest) GetIsolation() *TenantIsolation {
	if x != nil {
		return x.Isolation
	}
	return nil
}

func (x *ConfigureTenantIsolationRequest) SetMetadata(v *pb.RequestMetadata) {
	x.Metadata = v
}

func (x *ConfigureTenantIsolationRequest) SetTenantId(v string) {
	x.TenantId = &v
}

func (x *ConfigureTenantIsolationRequest) SetIsolation(v *TenantIsolation) {
	x.Isolation = v
}

func (x *ConfigureTenantIsolationRequest) HasMetadata() bool {
	if x == nil {
		return false
	}
	return x.Metadata != nil
}

func (x *ConfigureTenantIsolationRequest) HasTenantId() bool {
	if x == nil {
		return false
	}
	return x.TenantId != nil
}

func (x *ConfigureTenantIsolationRequest) HasIsolation() bool {
	if x == nil {
		return false
	}
	return x.Isolation != nil
}

func (x *ConfigureTenantIsolationRequest) ClearMetadata() {
	x.Metadata = nil
}

func (x *ConfigureTenantIsolationRequest) ClearTenantId() {
	x.TenantId = nil
}

func (x *ConfigureTenantIsolationRequest) ClearIsolation() {
	x.Isolation = nil
}

type ConfigureTenantIsolationRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Request metadata for tracing and context
	Metadata *pb.RequestMetadata
	// Tenant identifier
	TenantId *string
	// Isolation configuration to apply
	Isolation *TenantIsolation
}

func (b0 ConfigureTenantIsolationRequest_builder) Build() *ConfigureTenantIsolationRequest {
	m0 := &ConfigureTenantIsolationRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.Metadata = b.Metadata
	x.TenantId = b.TenantId
	x.Isolation = b.Isolation
	return m0
}

var File_pkg_organization_proto_requests_configure_tenant_isolation_request_proto protoreflect.FileDescriptor

const file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_rawDesc = "" +
	"\n" +
	"Hpkg/organization/proto/requests/configure_tenant_isolation_request.proto\x12\x17gcommon.v1.organization\x1a!google/protobuf/go_features.proto\x1a0pkg/common/proto/messages/request_metadata.proto\x1a3pkg/organization/proto/types/tenant_isolation.proto\"\xc6\x01\n" +
	"\x1fConfigureTenantIsolationRequest\x12>\n" +
	"\bmetadata\x18\x01 \x01(\v2\".gcommon.v1.common.RequestMetadataR\bmetadata\x12\x1b\n" +
	"\ttenant_id\x18\x02 \x01(\tR\btenantId\x12F\n" +
	"\tisolation\x18\x03 \x01(\v2(.gcommon.v1.organization.TenantIsolationR\tisolationB\xf9\x01\n" +
	"\x1bcom.gcommon.v1.organizationB$ConfigureTenantIsolationRequestProtoP\x01Z.../../../../pkg/organization/pb;organizationpb\xa2\x02\x03GVO\xaa\x02\x17Gcommon.V1.Organization\xca\x02\x17Gcommon\\V1\\Organization\xe2\x02#Gcommon\\V1\\Organization\\GPBMetadata\xea\x02\x19Gcommon::V1::Organization\x92\x03\x05\xd2>\x02\x10\x02b\beditionsp\xe8\a"

var file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_goTypes = []any{
	(*ConfigureTenantIsolationRequest)(nil), // 0: gcommon.v1.organization.ConfigureTenantIsolationRequest
	(*pb.RequestMetadata)(nil),              // 1: gcommon.v1.common.RequestMetadata
	(*TenantIsolation)(nil),                 // 2: gcommon.v1.organization.TenantIsolation
}
var file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_depIdxs = []int32{
	1, // 0: gcommon.v1.organization.ConfigureTenantIsolationRequest.metadata:type_name -> gcommon.v1.common.RequestMetadata
	2, // 1: gcommon.v1.organization.ConfigureTenantIsolationRequest.isolation:type_name -> gcommon.v1.organization.TenantIsolation
	2, // [2:2] is the sub-list for method output_type
	2, // [2:2] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_init() }
func file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_init() {
	if File_pkg_organization_proto_requests_configure_tenant_isolation_request_proto != nil {
		return
	}
	file_pkg_organization_proto_types_tenant_isolation_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_rawDesc), len(file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_goTypes,
		DependencyIndexes: file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_depIdxs,
		MessageInfos:      file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_msgTypes,
	}.Build()
	File_pkg_organization_proto_requests_configure_tenant_isolation_request_proto = out.File
	file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_goTypes = nil
	file_pkg_organization_proto_requests_configure_tenant_isolation_request_proto_depIdxs = nil
}
