// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: pkg/organization/proto/requests/list_tenants_request.proto

//go:build !protoopaque

package organizationpb

import (
	pb "../../../../pkg/common/pb"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ListTenantsRequest struct {
	state protoimpl.MessageState `protogen:"hybrid.v1"`
	// Request metadata for tracing and context
	Metadata *pb.RequestMetadata `protobuf:"bytes,1,opt,name=metadata" json:"metadata,omitempty"`
	// Organization identifier
	OrganizationId *string `protobuf:"bytes,2,opt,name=organization_id,json=organizationId" json:"organization_id,omitempty"`
	// Pagination size
	PageSize *int32 `protobuf:"varint,3,opt,name=page_size,json=pageSize" json:"page_size,omitempty"`
	// Pagination token from previous response
	PageToken *string `protobuf:"bytes,4,opt,name=page_token,json=pageToken" json:"page_token,omitempty"`
	// Optional filter expression
	Filter        *string `protobuf:"bytes,5,opt,name=filter" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTenantsRequest) Reset() {
	*x = ListTenantsRequest{}
	mi := &file_pkg_organization_proto_requests_list_tenants_request_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTenantsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTenantsRequest) ProtoMessage() {}

func (x *ListTenantsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_pkg_organization_proto_requests_list_tenants_request_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *ListTenantsRequest) GetMetadata() *pb.RequestMetadata {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *ListTenantsRequest) GetOrganizationId() string {
	if x != nil && x.OrganizationId != nil {
		return *x.OrganizationId
	}
	return ""
}

func (x *ListTenantsRequest) GetPageSize() int32 {
	if x != nil && x.PageSize != nil {
		return *x.PageSize
	}
	return 0
}

func (x *ListTenantsRequest) GetPageToken() string {
	if x != nil && x.PageToken != nil {
		return *x.PageToken
	}
	return ""
}

func (x *ListTenantsRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

func (x *ListTenantsRequest) SetMetadata(v *pb.RequestMetadata) {
	x.Metadata = v
}

func (x *ListTenantsRequest) SetOrganizationId(v string) {
	x.OrganizationId = &v
}

func (x *ListTenantsRequest) SetPageSize(v int32) {
	x.PageSize = &v
}

func (x *ListTenantsRequest) SetPageToken(v string) {
	x.PageToken = &v
}

func (x *ListTenantsRequest) SetFilter(v string) {
	x.Filter = &v
}

func (x *ListTenantsRequest) HasMetadata() bool {
	if x == nil {
		return false
	}
	return x.Metadata != nil
}

func (x *ListTenantsRequest) HasOrganizationId() bool {
	if x == nil {
		return false
	}
	return x.OrganizationId != nil
}

func (x *ListTenantsRequest) HasPageSize() bool {
	if x == nil {
		return false
	}
	return x.PageSize != nil
}

func (x *ListTenantsRequest) HasPageToken() bool {
	if x == nil {
		return false
	}
	return x.PageToken != nil
}

func (x *ListTenantsRequest) HasFilter() bool {
	if x == nil {
		return false
	}
	return x.Filter != nil
}

func (x *ListTenantsRequest) ClearMetadata() {
	x.Metadata = nil
}

func (x *ListTenantsRequest) ClearOrganizationId() {
	x.OrganizationId = nil
}

func (x *ListTenantsRequest) ClearPageSize() {
	x.PageSize = nil
}

func (x *ListTenantsRequest) ClearPageToken() {
	x.PageToken = nil
}

func (x *ListTenantsRequest) ClearFilter() {
	x.Filter = nil
}

type ListTenantsRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Request metadata for tracing and context
	Metadata *pb.RequestMetadata
	// Organization identifier
	OrganizationId *string
	// Pagination size
	PageSize *int32
	// Pagination token from previous response
	PageToken *string
	// Optional filter expression
	Filter *string
}

func (b0 ListTenantsRequest_builder) Build() *ListTenantsRequest {
	m0 := &ListTenantsRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.Metadata = b.Metadata
	x.OrganizationId = b.OrganizationId
	x.PageSize = b.PageSize
	x.PageToken = b.PageToken
	x.Filter = b.Filter
	return m0
}

var File_pkg_organization_proto_requests_list_tenants_request_proto protoreflect.FileDescriptor

const file_pkg_organization_proto_requests_list_tenants_request_proto_rawDesc = "" +
	"\n" +
	":pkg/organization/proto/requests/list_tenants_request.proto\x12\x17gcommon.v1.organization\x1a!google/protobuf/go_features.proto\x1a0pkg/common/proto/messages/request_metadata.proto\"\xd1\x01\n" +
	"\x12ListTenantsRequest\x12>\n" +
	"\bmetadata\x18\x01 \x01(\v2\".gcommon.v1.common.RequestMetadataR\bmetadata\x12'\n" +
	"\x0forganization_id\x18\x02 \x01(\tR\x0eorganizationId\x12\x1b\n" +
	"\tpage_size\x18\x03 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x04 \x01(\tR\tpageToken\x12\x16\n" +
	"\x06filter\x18\x05 \x01(\tR\x06filterB\xec\x01\n" +
	"\x1bcom.gcommon.v1.organizationB\x17ListTenantsRequestProtoP\x01Z.../../../../pkg/organization/pb;organizationpb\xa2\x02\x03GVO\xaa\x02\x17Gcommon.V1.Organization\xca\x02\x17Gcommon\\V1\\Organization\xe2\x02#Gcommon\\V1\\Organization\\GPBMetadata\xea\x02\x19Gcommon::V1::Organization\x92\x03\x05\xd2>\x02\x10\x02b\beditionsp\xe8\a"

var file_pkg_organization_proto_requests_list_tenants_request_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_pkg_organization_proto_requests_list_tenants_request_proto_goTypes = []any{
	(*ListTenantsRequest)(nil), // 0: gcommon.v1.organization.ListTenantsRequest
	(*pb.RequestMetadata)(nil), // 1: gcommon.v1.common.RequestMetadata
}
var file_pkg_organization_proto_requests_list_tenants_request_proto_depIdxs = []int32{
	1, // 0: gcommon.v1.organization.ListTenantsRequest.metadata:type_name -> gcommon.v1.common.RequestMetadata
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_pkg_organization_proto_requests_list_tenants_request_proto_init() }
func file_pkg_organization_proto_requests_list_tenants_request_proto_init() {
	if File_pkg_organization_proto_requests_list_tenants_request_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_pkg_organization_proto_requests_list_tenants_request_proto_rawDesc), len(file_pkg_organization_proto_requests_list_tenants_request_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_pkg_organization_proto_requests_list_tenants_request_proto_goTypes,
		DependencyIndexes: file_pkg_organization_proto_requests_list_tenants_request_proto_depIdxs,
		MessageInfos:      file_pkg_organization_proto_requests_list_tenants_request_proto_msgTypes,
	}.Build()
	File_pkg_organization_proto_requests_list_tenants_request_proto = out.File
	file_pkg_organization_proto_requests_list_tenants_request_proto_goTypes = nil
	file_pkg_organization_proto_requests_list_tenants_request_proto_depIdxs = nil
}
