// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.7
// 	protoc        (unknown)
// source: gcommon/v1/config/messages/inheritance_filter.proto

package messages

import (
	enums "github.com/jdfalk/gcommon/pkg/gcommon/v1/common/enums"
	enums1 "github.com/jdfalk/gcommon/pkg/gcommon/v1/config/enums"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type InheritanceFilter struct {
	state                  protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Type        enums.LogFilterType    `protobuf:"varint,1,opt,name=type,enum=gcommon.v1.common.LogFilterType"`
	xxx_hidden_Expression  *string                `protobuf:"bytes,2,opt,name=expression"`
	xxx_hidden_Action      enums1.FilterAction    `protobuf:"varint,3,opt,name=action,enum=gcommon.v1.config.FilterAction"`
	xxx_hidden_Metadata    map[string]string      `protobuf:"bytes,4,rep,name=metadata" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	XXX_raceDetectHookData protoimpl.RaceDetectHookData
	XXX_presence           [1]uint32
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *InheritanceFilter) Reset() {
	*x = InheritanceFilter{}
	mi := &file_gcommon_v1_config_messages_inheritance_filter_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *InheritanceFilter) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*InheritanceFilter) ProtoMessage() {}

func (x *InheritanceFilter) ProtoReflect() protoreflect.Message {
	mi := &file_gcommon_v1_config_messages_inheritance_filter_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *InheritanceFilter) GetType() enums.LogFilterType {
	if x != nil {
		if protoimpl.X.Present(&(x.XXX_presence[0]), 0) {
			return x.xxx_hidden_Type
		}
	}
	return enums.LogFilterType(0)
}

func (x *InheritanceFilter) GetExpression() string {
	if x != nil {
		if x.xxx_hidden_Expression != nil {
			return *x.xxx_hidden_Expression
		}
		return ""
	}
	return ""
}

func (x *InheritanceFilter) GetAction() enums1.FilterAction {
	if x != nil {
		if protoimpl.X.Present(&(x.XXX_presence[0]), 2) {
			return x.xxx_hidden_Action
		}
	}
	return enums1.FilterAction(0)
}

func (x *InheritanceFilter) GetMetadata() map[string]string {
	if x != nil {
		return x.xxx_hidden_Metadata
	}
	return nil
}

func (x *InheritanceFilter) SetType(v enums.LogFilterType) {
	x.xxx_hidden_Type = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 0, 4)
}

func (x *InheritanceFilter) SetExpression(v string) {
	x.xxx_hidden_Expression = &v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 1, 4)
}

func (x *InheritanceFilter) SetAction(v enums1.FilterAction) {
	x.xxx_hidden_Action = v
	protoimpl.X.SetPresent(&(x.XXX_presence[0]), 2, 4)
}

func (x *InheritanceFilter) SetMetadata(v map[string]string) {
	x.xxx_hidden_Metadata = v
}

func (x *InheritanceFilter) HasType() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 0)
}

func (x *InheritanceFilter) HasExpression() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 1)
}

func (x *InheritanceFilter) HasAction() bool {
	if x == nil {
		return false
	}
	return protoimpl.X.Present(&(x.XXX_presence[0]), 2)
}

func (x *InheritanceFilter) ClearType() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 0)
	x.xxx_hidden_Type = enums.LogFilterType_FILTER_TYPE_UNSPECIFIED
}

func (x *InheritanceFilter) ClearExpression() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 1)
	x.xxx_hidden_Expression = nil
}

func (x *InheritanceFilter) ClearAction() {
	protoimpl.X.ClearPresent(&(x.XXX_presence[0]), 2)
	x.xxx_hidden_Action = enums1.FilterAction_FILTER_ACTION_UNSPECIFIED
}

type InheritanceFilter_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Filter type
	Type *enums.LogFilterType
	// Filter expression
	Expression *string
	// Filter action
	Action *enums1.FilterAction
	// Filter metadata
	Metadata map[string]string
}

func (b0 InheritanceFilter_builder) Build() *InheritanceFilter {
	m0 := &InheritanceFilter{}
	b, x := &b0, m0
	_, _ = b, x
	if b.Type != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 0, 4)
		x.xxx_hidden_Type = *b.Type
	}
	if b.Expression != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 1, 4)
		x.xxx_hidden_Expression = b.Expression
	}
	if b.Action != nil {
		protoimpl.X.SetPresentNonAtomic(&(x.XXX_presence[0]), 2, 4)
		x.xxx_hidden_Action = *b.Action
	}
	x.xxx_hidden_Metadata = b.Metadata
	return m0
}

var File_gcommon_v1_config_messages_inheritance_filter_proto protoreflect.FileDescriptor

const file_gcommon_v1_config_messages_inheritance_filter_proto_rawDesc = "" +
	"\n" +
	"3gcommon/v1/config/messages/inheritance_filter.proto\x12\x11gcommon.v1.config\x1a)gcommon/v1/common/enums/filter_type.proto\x1a+gcommon/v1/config/enums/filter_action.proto\x1a!google/protobuf/go_features.proto\"\xaf\x02\n" +
	"\x11InheritanceFilter\x124\n" +
	"\x04type\x18\x01 \x01(\x0e2 .gcommon.v1.common.LogFilterTypeR\x04type\x12\x1e\n" +
	"\n" +
	"expression\x18\x02 \x01(\tR\n" +
	"expression\x127\n" +
	"\x06action\x18\x03 \x01(\x0e2\x1f.gcommon.v1.config.FilterActionR\x06action\x12N\n" +
	"\bmetadata\x18\x04 \x03(\v22.gcommon.v1.config.InheritanceFilter.MetadataEntryR\bmetadata\x1a;\n" +
	"\rMetadataEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01B\xd7\x01\n" +
	"\x15com.gcommon.v1.configB\x16InheritanceFilterProtoP\x01Z8github.com/jdfalk/gcommon/pkg/gcommon/v1/config/messages\xa2\x02\x03GVC\xaa\x02\x11Gcommon.V1.Config\xca\x02\x11Gcommon\\V1\\Config\xe2\x02\x1dGcommon\\V1\\Config\\GPBMetadata\xea\x02\x13Gcommon::V1::Config\x92\x03\x05\xd2>\x02\x10\x03b\beditionsp\xe8\a"

var file_gcommon_v1_config_messages_inheritance_filter_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_gcommon_v1_config_messages_inheritance_filter_proto_goTypes = []any{
	(*InheritanceFilter)(nil), // 0: gcommon.v1.config.InheritanceFilter
	nil,                       // 1: gcommon.v1.config.InheritanceFilter.MetadataEntry
	(enums.LogFilterType)(0),  // 2: gcommon.v1.common.LogFilterType
	(enums1.FilterAction)(0),  // 3: gcommon.v1.config.FilterAction
}
var file_gcommon_v1_config_messages_inheritance_filter_proto_depIdxs = []int32{
	2, // 0: gcommon.v1.config.InheritanceFilter.type:type_name -> gcommon.v1.common.LogFilterType
	3, // 1: gcommon.v1.config.InheritanceFilter.action:type_name -> gcommon.v1.config.FilterAction
	1, // 2: gcommon.v1.config.InheritanceFilter.metadata:type_name -> gcommon.v1.config.InheritanceFilter.MetadataEntry
	3, // [3:3] is the sub-list for method output_type
	3, // [3:3] is the sub-list for method input_type
	3, // [3:3] is the sub-list for extension type_name
	3, // [3:3] is the sub-list for extension extendee
	0, // [0:3] is the sub-list for field type_name
}

func init() { file_gcommon_v1_config_messages_inheritance_filter_proto_init() }
func file_gcommon_v1_config_messages_inheritance_filter_proto_init() {
	if File_gcommon_v1_config_messages_inheritance_filter_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_gcommon_v1_config_messages_inheritance_filter_proto_rawDesc), len(file_gcommon_v1_config_messages_inheritance_filter_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_gcommon_v1_config_messages_inheritance_filter_proto_goTypes,
		DependencyIndexes: file_gcommon_v1_config_messages_inheritance_filter_proto_depIdxs,
		MessageInfos:      file_gcommon_v1_config_messages_inheritance_filter_proto_msgTypes,
	}.Build()
	File_gcommon_v1_config_messages_inheritance_filter_proto = out.File
	file_gcommon_v1_config_messages_inheritance_filter_proto_goTypes = nil
	file_gcommon_v1_config_messages_inheritance_filter_proto_depIdxs = nil
}
