// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: pkg/db/proto/requests/run_migration_request.proto

//go:build !protoopaque

package dbpb

import (
	pb "../../../../pkg/common/pb"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	_ "google.golang.org/protobuf/types/gofeaturespb"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// *
// RunMigrationRequest represents a request to execute database migration scripts.
// Supports multiple scripts for complex migration scenarios.
type RunMigrationRequest struct {
	state protoimpl.MessageState `protogen:"hybrid.v1"`
	// Database name to run migrations against
	Database *string `protobuf:"bytes,1,opt,name=database" json:"database,omitempty"`
	// List of migration scripts to execute
	Scripts []*MigrationScript `protobuf:"bytes,2,rep,name=scripts" json:"scripts,omitempty"`
	// Request metadata for tracing and authentication
	Metadata      *pb.RequestMetadata `protobuf:"bytes,3,opt,name=metadata" json:"metadata,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RunMigrationRequest) Reset() {
	*x = RunMigrationRequest{}
	mi := &file_pkg_db_proto_requests_run_migration_request_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RunMigrationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RunMigrationRequest) ProtoMessage() {}

func (x *RunMigrationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_pkg_db_proto_requests_run_migration_request_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *RunMigrationRequest) GetDatabase() string {
	if x != nil && x.Database != nil {
		return *x.Database
	}
	return ""
}

func (x *RunMigrationRequest) GetScripts() []*MigrationScript {
	if x != nil {
		return x.Scripts
	}
	return nil
}

func (x *RunMigrationRequest) GetMetadata() *pb.RequestMetadata {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *RunMigrationRequest) SetDatabase(v string) {
	x.Database = &v
}

func (x *RunMigrationRequest) SetScripts(v []*MigrationScript) {
	x.Scripts = v
}

func (x *RunMigrationRequest) SetMetadata(v *pb.RequestMetadata) {
	x.Metadata = v
}

func (x *RunMigrationRequest) HasDatabase() bool {
	if x == nil {
		return false
	}
	return x.Database != nil
}

func (x *RunMigrationRequest) HasMetadata() bool {
	if x == nil {
		return false
	}
	return x.Metadata != nil
}

func (x *RunMigrationRequest) ClearDatabase() {
	x.Database = nil
}

func (x *RunMigrationRequest) ClearMetadata() {
	x.Metadata = nil
}

type RunMigrationRequest_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Database name to run migrations against
	Database *string
	// List of migration scripts to execute
	Scripts []*MigrationScript
	// Request metadata for tracing and authentication
	Metadata *pb.RequestMetadata
}

func (b0 RunMigrationRequest_builder) Build() *RunMigrationRequest {
	m0 := &RunMigrationRequest{}
	b, x := &b0, m0
	_, _ = b, x
	x.Database = b.Database
	x.Scripts = b.Scripts
	x.Metadata = b.Metadata
	return m0
}

var File_pkg_db_proto_requests_run_migration_request_proto protoreflect.FileDescriptor

const file_pkg_db_proto_requests_run_migration_request_proto_rawDesc = "" +
	"\n" +
	"1pkg/db/proto/requests/run_migration_request.proto\x12\x13gcommon.v1.database\x1a)pkg/db/proto/types/migration_script.proto\x1a0pkg/common/proto/messages/request_metadata.proto\x1a!google/protobuf/go_features.proto\"\xb9\x01\n" +
	"\x13RunMigrationRequest\x12\x1a\n" +
	"\bdatabase\x18\x01 \x01(\tR\bdatabase\x12B\n" +
	"\ascripts\x18\x02 \x03(\v2$.gcommon.v1.database.MigrationScriptB\x02(\x01R\ascripts\x12B\n" +
	"\bmetadata\x18\x03 \x01(\v2\".gcommon.v1.common.RequestMetadataB\x02(\x01R\bmetadataB\xc5\x01\n" +
	"\x17com.gcommon.v1.databaseB\x18RunMigrationRequestProtoP\x01Z\x1a../../../../pkg/db/pb;dbpb\xa2\x02\x03GVD\xaa\x02\x13Gcommon.V1.Database\xca\x02\x13Gcommon\\V1\\Database\xe2\x02\x1fGcommon\\V1\\Database\\GPBMetadata\xea\x02\x15Gcommon::V1::Database\x92\x03\x05\xd2>\x02\x10\x02b\beditionsp\xe8\a"

var file_pkg_db_proto_requests_run_migration_request_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_pkg_db_proto_requests_run_migration_request_proto_goTypes = []any{
	(*RunMigrationRequest)(nil), // 0: gcommon.v1.database.RunMigrationRequest
	(*MigrationScript)(nil),     // 1: gcommon.v1.database.MigrationScript
	(*pb.RequestMetadata)(nil),  // 2: gcommon.v1.common.RequestMetadata
}
var file_pkg_db_proto_requests_run_migration_request_proto_depIdxs = []int32{
	1, // 0: gcommon.v1.database.RunMigrationRequest.scripts:type_name -> gcommon.v1.database.MigrationScript
	2, // 1: gcommon.v1.database.RunMigrationRequest.metadata:type_name -> gcommon.v1.common.RequestMetadata
	2, // [2:2] is the sub-list for method output_type
	2, // [2:2] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_pkg_db_proto_requests_run_migration_request_proto_init() }
func file_pkg_db_proto_requests_run_migration_request_proto_init() {
	if File_pkg_db_proto_requests_run_migration_request_proto != nil {
		return
	}
	file_pkg_db_proto_types_migration_script_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_pkg_db_proto_requests_run_migration_request_proto_rawDesc), len(file_pkg_db_proto_requests_run_migration_request_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_pkg_db_proto_requests_run_migration_request_proto_goTypes,
		DependencyIndexes: file_pkg_db_proto_requests_run_migration_request_proto_depIdxs,
		MessageInfos:      file_pkg_db_proto_requests_run_migration_request_proto_msgTypes,
	}.Build()
	File_pkg_db_proto_requests_run_migration_request_proto = out.File
	file_pkg_db_proto_requests_run_migration_request_proto_goTypes = nil
	file_pkg_db_proto_requests_run_migration_request_proto_depIdxs = nil
}
